# RQ2: What is the causal association between the explanatory variable **exec** and the outcome of interest?
...
# Prepare multilvel GLM (exec + cover --> outcome)
dat_list <- list(
  killed = S$killed,
  exec = S$exec,
  cover = S$cover,
...
m <- ulam(
  alist(
    killed ~ dbinom(1, p),
    logit(p) <- a[prj] +  bExec[prj] * exec + bCover[prj] * cover,
    a[prj] ~ dnorm(0, 1),
...
saveRDS(m, file = "RQ2_m.rds")
m <- readRDS(file = "RQ2_m.rds")
